...
class APIKeyManager extends Component {
  ...
  fetchData = async () => {
    ...
    try {
      ...
      if (this.state.keyword != '') {
        url = `${apiKeys.list}?keyword=${this.state.keyword}&page=${this.state.current_page}`;
      } else {
        url = `${apiKeys.list}`;
      }
      ...
      if (
        ...
        const { api_keys, total_pages } = data;
      ...
      showErrorNotification('Lấy danh sách tất cả API key thất bại!');
    }
  };
  ...
  render() {
    ...
    const { apiKeys, current_page, total_pages } = this.state;
            ...
            <div className="row">
              ...
              <div className="right-column">
                <div className="form__header">
                  ...
                  <Link to="/bang-dieu-khien/api-key/them-moi" class="button">
                    Thêm mới
                  </Link>
                </div>
                      ...
                      {apiKeys &&
                        apiKeys.map((apiKey) => (
                          <tr>
                              ...
                              <div className="table-menu">
                                ...
                                <ul className="table-menu__list">
                                  <li className="table-menu__list-item">
                                    <Link
                                      to={`/bang-dieu-khien/api-key/thong-tin/${apiKey.id}`}
                                    >
                                      Xem thông tin
                                    </Link>
                                  </li>
                                  <li className="table-menu__list-item">
                                    <Link
                                      to={`/bang-dieu-khien/api-key/cap-nhat/${apiKey.id}`}
                                    >
                                      Cập nhật
                                    </Link>
                                  </li>
                                  <li className="table-menu__list-item">
                                    <Link
                                      to={`/bang-dieu-khien/api-key/xoa/${apiKey.id}`}
                                    >
                                      Xóa
                                    </Link>
                                  </li>
                            ...
                            <td>
                              {apiKey.user.full_name || apiKey.user.username}
                            </td>
                            <td>{apiKey.generated_key}</td>
                            <td>{formatDateString(apiKey.created_at)}</td>
                            <td>{apiKey.remaining_calls}</td>
                          </tr>
                        ))}
                    </table>
...
export default withRouter(
  connect(mapStateToProps, { showNotification })(APIKeyManager)
);
